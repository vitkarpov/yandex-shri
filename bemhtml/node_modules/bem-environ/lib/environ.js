var PATH = require('path'),
    ...
    /**
     * Путь до места, где (обычно?) лежит make.js
     * @const
     * @private
     */
    __bemRoot = PATH.dirname(require.resolve('../../../.bem/make.js')),
...
if(__root !== __bemRoot) {
    try {
        // есть ли у проекта `bem-environ` в зависимостях?
        matchEnviron(join(__root, '..', 'node_modules', 'bem-environ'));

        // Устанавливаем новое значение `__root_level_dir`
        process.env.__root_level_dir = __root;
    } catch(e) {
        // не нашли `bem-environ`, попробуем `.bem/environ`
        try {
            matchEnviron(join(__root, 'environ'));
...
function matchEnviron(newEnvironPath) {
    // Переопределяем константы `__extendables` из окружения проекта `ENV_ROOT`
    var rootEnviron = require(newEnvironPath);
